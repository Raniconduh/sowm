...
void win_fs(const Arg arg) {
    ...
    if ((cur->f = cur->f ? 0 : 1)) {
        ...
        XMoveResizeWindow(d, cur->w, mono_hgap, mono_vgap + bar_height, sw - (mono_hgap * 2), sh - (mono_vgap * 2) - bar_height);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for win {
            ...
            if ((c->f = c->f ? 0 : 1)) {
                ...
                XMoveResizeWindow(d, c->w, mono_hgap, mono_vgap + bar_height, sw - (mono_hgap * 2), sh - (mono_vgap * 2) - bar_height);
            // resize previous
            } else {
...
void win_fs_full(const Arg arg) {
    if (!cur) return;
...
int main(void) {
    ...
    bar.wh = bar_height;
    bar.w = XCreateSimpleWindow(d, root, 0, 0, sw, bar.wh, 0, bar_fg, bar_bg);
    ...
    XSetForeground(d, bar.gc, bar_fg);
